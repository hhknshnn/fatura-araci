...
def _load_countries_cfg():
    base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    path = os.path.join(base, 'config', 'countries.json')
...
try:
    ...
    print(f'countries.json yüklenemedi: {_e}')
    COUNTRIES_CFG = {}
    ...
    for r_idx, (_, row) in enumerate(df.iterrows()):
        ...
        for c_idx, (out_col, src_col) in enumerate(KZ_INV_COLS):
            ...
            if src_col == '__CALC__':
                ...
                dat(ws_inv, er, cn, parse_num(row.get(src_col, 0)), bg=bg, align='right', fmt=USD_FMT)
...
def generate_excel_ru(df, grup_kilolari, hedef_brut, exception_skus, logo_bytes,
    ...
    c = ws_inv.cell(row=gr, column=K, value='GRAND TOTAL USD')
    c.font = Font(name='Arial', bold=True, color='FFFFFF', size=11)
    c.fill = PatternFill('solid', fgColor=GOLD)
    c.alignment = Alignment(horizontal='center', vertical='center')
    c.border = brd()
...
def _dispatch_eur_standard(ulke_kodu, df, grup_kilolari, hedef_brut,
                            exception_skus, logo_bytes, pdf_fields,
                            eur_kuru, **kw):
    if ulke_kodu == 'be':
        excel_out, fatura_no, master_out, mill_test_out = generate_excel_be(
            df, grup_kilolari, hedef_brut, exception_skus,
            logo_bytes, pdf_fields, eur_kuru=eur_kuru, **kw)
        return excel_out, fatura_no, master_out, None, mill_test_out
    else:
        template_map = {
            'de': (find_de_template_path, apply_de_template_header, DE_INV_COLS, DE_PL_COLS),
            'nl': (find_nl_template_path, apply_nl_template_header, NL_INV_COLS, NL_PL_COLS),
            'xk': (find_ko_template_path, apply_ko_template_header, KO_INV_COLS, KO_PL_COLS),
            'mk': (find_mk_template_path, apply_mk_template_header, KO_INV_COLS, KO_PL_COLS),
        }
        find_fn, header_fn, inv_cols, pl_cols = template_map[ulke_kodu]
        freight   = float((pdf_fields or {}).get('navlun',  0) or 0) / eur_kuru
        insurance = float((pdf_fields or {}).get('sigorta', 0) or 0) / eur_kuru
        excel_out, fatura_no, master_out = _generate_excel_eur(
            df, grup_kilolari, hedef_brut, exception_skus,
            pdf_fields, kw['hedef_net'], kw['depo_tipi'], eur_kuru,
            freight, insurance, find_fn, header_fn,
            inv_cols=inv_cols, pl_cols=pl_cols,
            df_original=kw['df_original'])
        return excel_out, fatura_no, master_out, None, None

def _dispatch_kz_standard(ulke_kodu, df, grup_kilolari, hedef_brut,
                           exception_skus, logo_bytes, pdf_fields, **kw):
    if ulke_kodu == 'kz':
        excel_out, fatura_no, master_out, price_list_out = generate_excel_kz(
            df, grup_kilolari, hedef_brut, exception_skus,
            logo_bytes, pdf_fields, **kw)
        ...
        excel_out, fatura_no, master_out = generate_excel_uz(
            ...
            logo_bytes, pdf_fields, **kw)
        return excel_out, fatura_no, master_out, None, None
    raise NotImplementedError(f'kz_standard: bilinmeyen ülke {ulke_kodu}')

def _dispatch_usd_standard(ulke_kodu, df, grup_kilolari, hedef_brut,
                            exception_skus, pdf_fields, usd_kuru, **kw):
    template_map = {
    ...
    find_fn = template_map.get(ulke_kodu)
    if not find_fn:
        raise NotImplementedError(f'usd_standard: bilinmeyen ülke {ulke_kodu}')
    excel_out, fatura_no, master_out = _generate_excel_usd(
        df, grup_kilolari, hedef_brut, exception_skus,
        pdf_fields, kw['hedef_net'], kw['depo_tipi'], usd_kuru,
        find_fn, df_original=kw['df_original'])
    return excel_out, fatura_no, master_out, None, None

def dispatch_generate(ulke_kodu, df, df_original, grup_kilolari, hedef_brut,
                      hedef_net, depo_tipi, exception_skus, logo_bytes,
                      pdf_fields, eur_kuru=1.0, usd_kuru=1.0):
    cfg    = COUNTRIES_CFG.get(ulke_kodu, {})
    engine = cfg.get('engine')
    kw     = dict(hedef_net=hedef_net, depo_tipi=depo_tipi, df_original=df_original)

    if engine == 'eur_standard':
        return _dispatch_eur_standard(
            ulke_kodu, df, grup_kilolari, hedef_brut, exception_skus,
            logo_bytes, pdf_fields, eur_kuru, **kw)
    elif engine == 'rs_standard':
        excel_out, fatura_no, master_out = generate_excel(
            df, grup_kilolari, hedef_brut, exception_skus,
            logo_bytes, pdf_fields, **kw)
        return excel_out, fatura_no, master_out, None, None
    elif engine == 'ge_standard':
        excel_out, fatura_no, master_out = generate_excel_ge(
            df, grup_kilolari, hedef_brut, exception_skus,
            logo_bytes, pdf_fields, **kw)
        return excel_out, fatura_no, master_out, None, None
    elif engine == 'ba_standard':
        excel_out, fatura_no, master_out = generate_excel_ba(
            df, grup_kilolari, hedef_brut, exception_skus,
            logo_bytes, pdf_fields, **kw)
        return excel_out, fatura_no, master_out, None, None
    elif engine == 'kz_standard':
        return _dispatch_kz_standard(
            ulke_kodu, df, grup_kilolari, hedef_brut, exception_skus,
            logo_bytes, pdf_fields, **kw)
    elif engine == 'usd_standard':
        return _dispatch_usd_standard(
            ulke_kodu, df, grup_kilolari, hedef_brut, exception_skus,
            pdf_fields, usd_kuru, **kw)
    else:
        raise NotImplementedError(f'Bilinmeyen engine: {engine} ({ulke_kodu})')
